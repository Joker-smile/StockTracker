...
### 2. 智能防呆交互
- **纯数字输入/手动前缀**：添加股票时默认支持 `6位数字代码` 自动识别前缀。特殊品种（如指数）可手动加前缀，如输入 `sh000001` (上证指数) 或 `sz399001` (深证成指) 以精准识别。
- **开盘前智能显示**：修复了开盘前或未成交时涨跌幅显示为 `-100%` 的问题，现在会智能显示为 `0.00%`。
...
| 状态词 | 盘口含义 |
| :--- | :--- |
| **高位滞涨(防跳水)** | 价格不涨但成交量剧增，多头力竭，需警惕主力派发。 |
| **强势回落(待观察)** | 冲高后遇阻回落，多空分歧加剧。 |
| **回踩确认(洗盘结束)** | 价格缩量回调至 5 日线附近获得支撑。 |
| **缩量洗盘** | 波动收窄，量能萎缩，趋势暂不明确，可能是主力洗出不坚定筹码。 |
| **弱势抵抗** | 缩量下影线，跌势末端的小幅反抗，尚未企稳。 |
| **震荡洗盘** | 在一定区间内反复震荡，清洗获利盘。 |
| **蓄势震荡** | 成交量温和，主力在当前价位进行筹码交换。 |
| **弱势震荡** | 处于均线下方且走势疲软，交投清淡。 |
| **放量承接** | 跌势中伴随放量，表明底部有一定资金买入支撑，但尚未反转。 |
...
## 🧠 智测计算规则 (Algorithm Rules)

“智测”系统基于实时行情与历史 K 线数据，采取以下多维度算法进行综合识别：

1. **五日均线 (MA5) 判定**：
   - 算法：`(前4日收盘价总和 + 当前价) / 5`。
   - 作用：识别当前股价相对于短期平均成本的位置（乖离率）。

2. **动态量比 (Dynamic Volume Ratio)**：
   - 算法：`(当日成交量 / 前4日均量) / 当日已交易时间占比`。
   - 修正：根据 A 股开盘时间（9:30-11:30, 13:00-15:00）动态计算时间进度，确保早盘量比的准确性。

3. **近期趋势 (Trend Analysis)**：
   - 算法：分析前 4 个交易日的收盘价斜率。
   - 作用：识别是“超跌反弹”还是“顺势波段”。

4. **K 线形态识别 (Pattern Detection)**：
   - **避雷针**：长上影线 + 股价处于高位 + 阴线。
   - **仙人指路**：长上影线 + 处于上升通道 + 价格回踩 MA5 + 缩量。
   - **金针探底**：长下影线 + 低位爆量承接。

5. **全品种板块识别**：
   - 自动适配：**指数** (上证/深证)、**ETF** (沪/深)、**LOF** (深/沪)、**REITs**、**可转债** 以及各交易板块 (主板 10%，创业板/科创板 20%，北交所 30%) 的涨跌停阈值与盘口语境。

6. **核心逻辑速查表**：
   - **[看多]**：爆量主升、缩量逼空(控盘)、多头掌控、震荡攀升、仙人指路、金针探底。
   - **[观察]**：高位滞涨(防跳水)、缩量洗盘、强势回落、弱势抵抗、震荡洗盘、数据不足、弱势震荡、放量承接。
   - **[看空]**：破位杀跌、阴跌不止、情绪雪崩、恐慌跌停、抛压巨大(避雷针)。
